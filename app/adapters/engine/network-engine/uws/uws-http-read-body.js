// adapters/engine/network-engine/uws/uws-http-read-body.js
...
function readBody(executionContext) {
  return new Promise(async (resolve, reject) => {
    ...
    const { config, maxInputBytes } = networkEngine.requestPipeline;

    const MAX_INPUT_BYTES = parseBytes(tenantRoute.maxInputBytes ?? maxInputBytes);

    // SENDING DATA RUNS TENANT SESSION / CSRF HANDLER
    const authCSRF = await sessionHelper.authSessionCSRF();
    if (!isAuthorized(authCSRF)) return reject("401 Unauthorized");
    ...
    if (preloadedBodyPromise) {
      try {
        ...
        if (contentType.includes(CONTENT_TYPE_MULTIPART)) {
          const UPLOAD_PATH = tenantRoute.uploadPath ?? config.uploadPath;
