...
const BASELINE_MIX = Object.freeze([
  ...
  { profile: `controller`, weight: 40 },
  { profile: `sessionController`, weight: 10 },
]);
...
const PHASE_MIX_OVERRIDES = Object.freeze({
  spike: [
    ...
    { profile: `controller`, weight: 55 },
    { profile: `sessionController`, weight: 15 },
  ]
});
...
function pickWeighted(mix) {
  ...
  if (total <= 0) return mix[0]?.profile ?? `controller`;
  let ticket = Math.random() * total;
  for (const item of mix) {
    ticket -= Math.max(0, Number(item.weight ?? 0));
    if (ticket <= 0) return item.profile;
  }
  return mix[mix.length - 1]?.profile ?? `controller`;
}
...
async function run() {
  ...
  const includeProfiles = String(args.profiles ?? `staticAsset,cacheHit,controller,sessionController`)
    .split(`,`)
  ...
  for (const profile of includeProfiles) {
    scenarioPaths[profile] = DEFAULT_PATHS[profile] ?? DEFAULT_PATHS.controller;
  }
  ...
  async function issueRequest(phaseName, workerId) {
    ...
    const requestPath = scenarioPaths[profile] ?? DEFAULT_PATHS.controller;
