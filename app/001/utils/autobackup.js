if(global.config.autobackup.enabled){
	...
	global.setInterval(async ()=>{
		...
		console.log(`# BACKUP CLEANING 1 PER DAY`);
		...
		for(const entry of entries) { 
			...
			old_backups[match.groups.date].push(entry.name);
		...
		for(const d in old_backups){
			//for(const t in old_backups[d]){
				for(let i=0, l=old_backups[d].length-1; i<l; i++){
					const file = `${global.config.autobackup.output}/${old_backups[d][i]}`;
	...
	}, (global.config.autobackup.interval??3600000)*3, false);
