...
const available_methods = ["GET", "POST", "PUT", "DELETE", "OPTIONS", "PATCH", /*"HEAD", "CONNECT", "TRACE"*/];

global.ipcheck = global.ipcheck ?? require('ip-range-check');

global.errored_ips_file = global.cache_path_resolve('errored_ips.json');
global.errored_ips = global.path.get_json(global.errored_ips_file);

global.blocked_ips_file = global.cache_path_resolve('blocked_ips.json');
...
exports.checkBlock = async function check(req, res){
  ...
  if(available_methods.indexOf(req.method) == -1){
    res.status(400).end();
    return true;
  }

  if(req.url.startsWith(`/clear_errors_ip`)){
  ...
  //CHECK ALLOWED IPS
  if(!global.ipcheck( req.connection.remoteAddress,  global.config.limiter.allowed_origin_ips )) { 
    log("~~ ACCESS FROM NON ALLOWED IP ~~ BLOCKING IN LOCAL FIREWALL");
    ...
    global.default403(req, res, "INVALID ALLOWED IP");
    return true;
  }
