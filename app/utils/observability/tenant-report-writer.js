...
function createTenantReportWriter(options = {}) {
  ...
  function observeRequest(executionContext) {
    ...
    const tenantHost = String(tenantRoute?.host ?? ``).trim();
    const tenantRoot = String(tenantRoute?.rootFolder ?? ``).trim();
...
function normalizeTenantReportRelativePath(relativePath) {
  const fallback = path.posix.join(`src`, `report.json`);
  ...
  if (parts[0] !== `src`) {
    return path.posix.join(`src`, path.posix.basename(sanitized));
  }
