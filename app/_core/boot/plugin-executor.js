...
class PluginExecutor {
  ...
  constructor(processLabel = null, pluginsConfig = {}) {
    ...
    this.listeners = new Array(pluginHooks.MAX_HOOKS); // sparse
  ...
  async runWithContext(hook, context, { errHook = null, rethrow = false } = {}) {
    ...
    for (const h of list) {
      ...
      try {
        await Promise.resolve().then(() => h.fn(hookContext));
      } catch (error) {
  ...
  async unload(pluginName, {
    ...
    for (const k in this.listeners) {
      ...
      if (!list) continue;
      this.listeners[k] = list.filter(h => h.plugin !== pluginName);
    }
    this.plugins.delete(pluginName);
  }

  /** Loads plugins for the current process context and configures hook access ranges. */
  async scanPlugins(contextName, customPluginsPath, pluginsConfig = {}) {
    ...
    this.pluginsConfig = pluginsConfig ?? this.pluginsConfig ?? {};
    const pluginScan = require(`@/utils/plugin-scan`);
    this.plugins = await pluginScan(this, customPluginsPath, pluginsConfig, contextName)
