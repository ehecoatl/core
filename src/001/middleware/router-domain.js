...
async function mid_domain(req, res) {
    ...
    req.version = global.router.get_route_version(`${req.request_sub}.${req.request_domain}`);

    req.dom_path = `${global.config.web_folder}/${req.request_domain}`;
    req.root_path = `${global.config.web_folder}/${req.request_domain}/${req.request_sub}/${req.version}`;
