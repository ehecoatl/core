...
# 4. Update the shared cache configuration for Redis.
...
SHARED_CACHE_CONFIG_FILE="$ETC_CONFIG_DIR/sharedCacheService.json"
...
log() { printf '%s[EHECATL BOOTSTRAP REDIS]%s %s\n' "$LOG_PREFIX_STYLE" "$LOG_RESET_STYLE" "$1"; }
fail() {
...
write_shared_cache_service_config() {
  if [ "$DRY_RUN" -eq 1 ]; then
    log "[dry-run] ensure $SHARED_CACHE_CONFIG_FILE contains adapter=redis"
    return 0
  fi

  run_quiet $SUDO mkdir -p "$ETC_CONFIG_DIR"
...
'
  ...
  local output
  if ! output="$(node -e "$node_script" "$SHARED_CACHE_CONFIG_FILE" 2>&1)"; then
    fail "$output"
  fi
}
...
parse_args "$@"
if ! $SUDO test -f "$INSTALL_META_FILE"; then fail "Install metadata was not found at $INSTALL_META_FILE. Run setup/setup-ehecatl.sh first."; fi
...
if [ "$DRY_RUN" -eq 1 ]; then
  ...
  log "  - Ensure $SHARED_CACHE_CONFIG_FILE uses adapter=redis while preserving other JSON properties"
  exit 0
fi
