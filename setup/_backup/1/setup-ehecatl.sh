...
step "Creating runtime group"; if ! getent group "$EHECATL_GROUP" >/dev/null 2>&1; then run_quiet $SUDO groupadd --system "$EHECATL_GROUP"; else log "System group '$EHECATL_GROUP' already exists."; fi
step "Creating runtime user"; if ! id "$EHECATL_USER" >/dev/null 2>&1; then run_quiet $SUDO useradd --system --gid "$EHECATL_GROUP" --no-create-home --shell /usr/sbin/nologin "$EHECATL_USER"; else log "System user '$EHECATL_USER' already exists."; run_quiet $SUDO usermod -g "$EHECATL_GROUP" "$EHECATL_USER"; run_quiet $SUDO usermod -a -G "$EHECATL_GROUP" "$EHECATL_USER"; fi
for child_user in "${DEFAULT_CHILD_USERS[@]}"; do if ! id "$child_user" >/dev/null 2>&1; then run_quiet $SUDO useradd --system --gid "$EHECATL_GROUP" --no-create-home --shell /usr/sbin/nologin "$child_user"; else log "System user '$child_user' already exists."; run_quiet $SUDO usermod -g "$EHECATL_GROUP" "$child_user"; run_quiet $SUDO usermod -a -G "$EHECATL_GROUP" "$child_user"; fi; done
step "Publishing CLI command"; run_quiet $SUDO chmod +x "$CLI_BASE_DIR/ehecatl"; run_quiet $SUDO ln -sfn "$CLI_BASE_DIR/ehecatl" "$CLI_TARGET"
...
step "Finishing"; log "Ehecatl installed successfully."; log "Use 'ehecatl start' to launch manually when needed."; log "Use setup/bootstrap-redis.sh only when you want Ehecatl to manage a local Redis ${EHECATL_REDIS_SUPPORTED_MAJOR:-7}.x installation."
