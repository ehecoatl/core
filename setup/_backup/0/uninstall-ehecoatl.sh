...
# 6. Remove the published Ehecoatl CLI symlink.
...
verify_uninstall_state() {
  ...
  if command -v pm2 >/dev/null 2>&1; then
    if $SUDO pm2 describe Ehecoatl >/dev/null 2>&1; then
      fail "PM2 app entry 'Ehecoatl' still exists after uninstall."
    fi
  fi
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  command -v sudo >/dev/null 2>&1 || fail "sudo is required to uninstall Ehecoatl."
  SUDO="sudo"
fi
...
log "To remove custom data too, run purge-ehecoatl-data.sh from another Ehecoatl checkout before or after reinstalling."
