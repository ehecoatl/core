...
# 6. Offer to move the project into /opt/ehecatl when running elsewhere.
...
# 13. Publish the Ehecatl CLI via a symlink in /usr/local/bin.
# 14. Create the standard /var, /srv, and /etc directory layout.
# 15. Apply ownership and permissions to the standard directories.
# 16. Install and enable the systemd service unit for Ehecatl.
# 17. Write installation metadata to /etc/opt/ehecatl/install-meta.env.
...
if [ "$(id -u)" -eq 0 ]; then
  ...
  command -v sudo >/dev/null 2>&1 || fail "sudo is required to install Ehecatl."
  SUDO="sudo"
fi
...
check_nodejs_24() {
  ...
  log "Please install Node.js 24 before running Ehecatl."
  return 1
}

# Stage 5: install non-Node system dependencies required by Ehecatl and npm native builds.
...
load_runtime_policy() {
  ...
  EHECATL_USER="$(policy_value 'system.sharedUser')"
  EHECATL_GROUP="$(policy_value 'system.sharedGroup')"
...
apply_standard_directory_permissions() {
  ...
  for managed_dir in "${managed_dirs[@]}"; do
    apply_owner_group_mode "$managed_dir" "$EHECATL_USER" "$EHECATL_GROUP" 2775
  done
}

ensure_maintenance_scripts_executable() {
  local maintenance_scripts=(
    "$SCRIPT_DIR/setup-ehecatl.sh"
    "$SCRIPT_DIR/uninstall-ehecatl.sh"
    "$SCRIPT_DIR/purge-ehecatl-data.sh"
  )
  local script_path
...
grant_project_runtime_access() {
  ...
  local runtime_users=("$EHECATL_USER" "${DEFAULT_CHILD_USERS[@]}")
...
REDIS_PACKAGE_NAME="${EHECATL_REDIS_PACKAGE_NAME:-}"
REDIS_SERVICE_NAME="${EHECATL_REDIS_SERVICE_NAME:-}"
REDIS_MANAGED_BY_INSTALLER="${EHECATL_REDIS_MANAGED_BY_INSTALLER:-0}"
EOF
  )
  ...
  apply_owner_group_mode "$INSTALL_META_FILE" "$EHECATL_USER" "$EHECATL_GROUP" 640
}
...
if detect_existing_install; then
  if [ "$FORCE_INSTALL" -eq 0 ]; then
    ...
    log "Run setup/setup-ehecatl.sh --force to reapply setup and runtime service provisioning."
    exit 0
  fi
...
# Stage 13: publish the Ehecatl CLI into the system PATH.
step "Publishing CLI command"
run_quiet $SUDO chmod +x "$CLI_BASE_DIR/ehecatl"
run_quiet $SUDO ln -sfn "$CLI_BASE_DIR/ehecatl" "$CLI_TARGET"
...
step "Finishing"
log "Ehecatl installed."
log "CLI available at: $CLI_TARGET"
log "Project directory: $PROJECT_DIR"
log "Runtime user: $EHECATL_USER"
log "Runtime service: $SYSTEMD_UNIT_NAME (enabled and active)"
log "To uninstall binaries/project, run: ./setup/uninstall-ehecatl.sh"
log "To remove custom data, run: ./setup/purge-ehecatl-data.sh"
