...
init_runtime_policy_helper(){ [ -f "$RUNTIME_POLICY_HELPER" ] || return 1; source "$RUNTIME_POLICY_HELPER"; policy_init "$INSTALL_DIR/cli/ehecoatl.sh"; }
...
require_root(){ if [ "$DRY_RUN" -eq 1 ]; then return 0; fi; if [ "$(id -u)" -eq 0 ]; then return 0; fi; if command -v sudo >/dev/null 2>&1; then fail "uninstall-ehecoatl.sh must be run as root or invoked via sudo."; fi; fail "uninstall-ehecoatl.sh must be run as root. sudo is not available on this host."; }
SUDO=""
...
POLICY_PROJECT_DIR="${PROJECT_DIR:-$DEFAULT_PROJECT_DIR}"; POLICY_FILE="$POLICY_PROJECT_DIR/config/runtime-policy.json"
