...
print_help() {
  cat <<'EOF'
Usage: setup/uninstall-ehecoatl.sh [options]
...
require_root(){ if [ "$DRY_RUN" -eq 1 ]; then return 0; fi; if [ "$(id -u)" -eq 0 ]; then return 0; fi; if command -v sudo >/dev/null 2>&1; then [ "${EHECOATL_SETUP_SUDO_REEXEC:-0}" = "1" ] && fail "uninstall-ehecoatl.sh could not acquire root privileges through sudo."; exec sudo EHECOATL_SETUP_SUDO_REEXEC=1 bash "$0" "${SCRIPT_ARGS[@]}"; fi; fail "uninstall-ehecoatl.sh must be run as root. sudo is not available on this host."; }
SUDO=""
