...
# 4. Optionally invoke setup/setup-ehecoatl.sh.
...
print_help() {
  cat <<'EOF'
Usage: setup/bootstrap-ehecoatl.sh [options]
...
Options:
  ...
  --auto-installer    Run setup/setup-ehecoatl.sh after bootstrap finishes.
  --complete          Alias for --auto-installer for full bootstrap flow.
...
require_root() {
  ...
  if command -v sudo >/dev/null 2>&1; then
    [ "${EHECOATL_SETUP_SUDO_REEXEC:-0}" = "1" ] && fail "bootstrap-ehecoatl.sh could not acquire root privileges through sudo."
