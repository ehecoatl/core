...
cleanup_table_chains() {
  ...
  for chain in $chains; do
    if [ "$table" = "filter" ]; then
      ...
      run_rule sudo iptables -t "$table" -D OUTPUT -m owner --uid-owner ehecatl -j "$chain"
    else
      run_rule sudo iptables -t "$table" -D OUTPUT -j "$chain"
    fi
...
log "Releasing every Ehecatl proxy rule"

cleanup_output_matches "filter" "EHECATL_PROXY_FILTER_"
cleanup_output_matches "nat" "EHECATL_PROXY_NAT_"
cleanup_table_chains "filter" "EHECATL_PROXY_FILTER_"
cleanup_table_chains "nat" "EHECATL_PROXY_NAT_"
