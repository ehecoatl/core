// _core/runtimes/request-uri-route-resolver/request-uri-route-resolver.js
...
class RequestUriRouteResolver extends AdaptableUseCase {
  config;
  adapter;
  storageService;
  sharedCacheService;
  ...
  constructor(kernelContext) {
    super(kernelContext.config._adapters.requestUriRouteResolver);
    this.config = kernelContext.config.adapters.requestUriRouteResolver ?? {};
  ...
  async matchRoute({ url, tenantId = null, forcedAppId = null }) {
    const cacheKey = buildRouteCacheKey({ url, tenantId, forcedAppId });
...
module.exports = RequestUriRouteResolver;
