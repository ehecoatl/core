...
DIRECTOR_PROCESS_JSON="$(node "$CONTRACT_IDENTITY_CLI" process-identity supervisionScope director)"
DIRECTOR_USER="$(json_field "$DIRECTOR_PROCESS_JSON" user 2>/dev/null || true)"
usage() {
  cat <<'EOF_USAGE'
...
apply_app_permissions() {
  ...
  make_tree_public_readable "$asset_static_dir" "$asset_static_mode" "$asset_static_file_mode"
...
apply_tenant_permissions() {
  ...
  local owner_group="$3"
  local tenant_id="$4"
  local asset_static_json shared_assets_static_dir shared_assets_static_mode shared_assets_static_file_mode
  ...
  shared_assets_static_mode="$(json_field "$asset_static_json" mode 2>/dev/null || true)"
  [ -n "$shared_assets_static_mode" ] || shared_assets_static_mode="2775"
  shared_assets_static_file_mode="$(dir_mode_to_file_mode "$shared_assets_static_mode")"

  [ -d "$tenant_dir" ] || return 0
  sudo chown -R "$owner_user:$owner_group" "$tenant_dir"
  sudo find "$tenant_dir" -type d -exec chmod 2770 {} +
  sudo find "$tenant_dir" -type f -exec chmod 0660 {} +
  set_owner_group_mode "$tenant_dir" "$owner_user" "$owner_group" "2751"
  [ -d "$tenant_dir/shared" ] && set_owner_group_mode "$tenant_dir/shared" "$owner_user" "$owner_group" "2751"
  [ -d "$tenant_dir/shared/assets" ] && set_owner_group_mode "$tenant_dir/shared/assets" "$owner_user" "$owner_group" "2751"
  make_tree_public_readable "$shared_assets_static_dir" "$shared_assets_static_mode" "$shared_assets_static_file_mode"
}

apply_recursive_read_acl() {
  local target_path="$1"
  local user_name="$2"
  ...
  if [ -d "$target_path" ]; then
    sudo setfacl -R -m "u:${user_name}:rX" "$target_path" >/dev/null 2>&1 || true
    sudo setfacl -R -d -m "u:${user_name}:rX" "$target_path" >/dev/null 2>&1 || true
    return 0
  fi

  sudo setfacl -m "u:${user_name}:rX" "$target_path" >/dev/null 2>&1 || true
}
...
deploy_tenant() {
  ...
  apply_recursive_read_acl "$tenant_dir" "$DIRECTOR_USER"
...
deploy_app() {
  ...
  local normalized_target target_app_name target_domain target_tenant_id target_mode tenant_json tenant_dir tenant_id app_json app_id app_dir app_user app_group app_owner app_owner_group app_shell_json app_fs_json app_kit_dir tenant_host selected_app_kit_name
  normalized_target="$(normalize_lower "$TARGET_ALIAS")"
  ...
  apply_recursive_read_acl "$app_dir" "$DIRECTOR_USER"
