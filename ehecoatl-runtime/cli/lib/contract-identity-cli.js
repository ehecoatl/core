...
function main() {
  switch (command) {
    case `tenant-filesystem`:
      return outputJson(contractIdentity.getRenderedTenantFilesystemIdentity(args[0], args[1] ?? null));
    case `app-filesystem`:
      return outputJson(contractIdentity.getRenderedAppFilesystemIdentity(args[0], args[1], args[2] ?? null, args[3] ?? null));
    case `shell-identity`:
      return outputJson(contractIdentity.getRenderedScopeShellIdentity(args[0], {
        tenantId: args[1] ?? null,
        appId: args[2] ?? null,
        tenantDomain: args[3] ?? null,
        appName: args[4] ?? null,
