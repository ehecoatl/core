...
Defines ACL rules for director, transport, and isolated-runtime access into tenant folders. The bundled policy grants director read access to `config.json` and static-asset reads from `assets`, while transport and isolated-runtime access are scoped to `assets` and writable runtime folders under `.ehecoatl/` such as `.ehecoatl/.cache`, `.ehecoatl/.log`, `.ehecoatl/.spool`, and `.ehecoatl/.backups`. Tenant provisioning scripts consume these rules to apply read and write access for the correct runtime users.
