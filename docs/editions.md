...
When a page describes a feature, it is based on the code currently available here. If a capability depends on custom plugins, custom adapters, or other repository areas not present in this snapshot, the docs call that out explicitly instead of treating it as shipped built-in behavior.
